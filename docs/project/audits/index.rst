============================
Audit & Traceability Matrix
============================

Bidirectional Traceability Matrix
---------------------------------

.. list-table::
   :widths: 15 25 35 25
   :header-rows: 1

   * - Functional ID
     - Technical ID
     - Description
     - Roadmap Milestone
   * - **[FUNC-001]**
     - **[TECH-001]**
     - CLI Invocation & Backward Compatibility
     - Phase 1 (Milestone 1.2)
   * - **[FUNC-002]**
     - **[TECH-002]**
     - Multi-Platform Template Discovery
     - Phase 1 (Milestone 1.2)
   * - **[FUNC-003]**
     - **[TECH-003]**
     - Directory Tree Mirroring Engine
     - Phase 1 (Milestone 1.1)
   * - **[FUNC-004]**
     - **[TECH-004]**
     - Token Replacement & Macro Collision Guard
     - Phase 1 (Milestone 1.1)
   * - **[FUNC-005]**
     - **[TECH-005]**
     - Binary Asset & Media Passthrough
     - Phase 1 (Milestone 1.1)
   * - **[FUNC-006]**
     - **[TECH-005]**
     - POSIX Permission Preservation
     - Phase 1 (Milestone 1.1)
   * - **[FUNC-007]**
     - **[TECH-006]**
     - Optional Template Manifest (`template.yml`)
     - Phase 1 (Milestone 1.2)
   * - **[FUNC-008]**
     - **[TECH-006]**
     - Automated Git Repository Initialization
     - Phase 1 (Milestone 1.2)

Protocol Audits
---------------

.. toctree::
   :maxdepth: 1

   2026-09-18-crinit-echelon-review

